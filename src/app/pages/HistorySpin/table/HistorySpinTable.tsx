...
import { HistorySpinColumns } from './columns/_columns'
...
const HistorySpinTable: FC<Props> = ({ namespace }) => {
  const historySpins = useQueryResponseData(namespace)
  ...
  const columns = useMemo<ColumnDef<any>[]>(() => HistorySpinColumns(namespace), [namespace])
          ...
          <tbody className='text-gray-600'>
            {table.getRowModel().rows.length > 0 ? (
              table.getRowModel().rows.map((row) => (
                <tr
                  key={row.id}
                  className={row.original.Quantity === 0 ? 'bg-danger-subtle' : ''}
                >
