...
import { PrizesColumns } from './columns/_columns'
...
const PrizesTable: FC<Props> = ({ namespace }) => {
  const Prizess = useQueryResponseData(namespace)
  ...
  const columns = useMemo<ColumnDef<any>[]>(() => PrizesColumns(namespace), [namespace])
          ...
          <tbody className='text-gray-600'>
            {table.getRowModel().rows.length > 0 ? (
              table.getRowModel().rows.map((row) => (
                <tr
                  key={row.id}
                  className={(row.original.Quantity === 0 && !row.original.IsUnlimited) ? 'bg-danger-subtle' : ''}
                >
